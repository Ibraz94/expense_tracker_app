    ...
    <div className="flex flex-col h-screen">
      ...
      <main className="flex-1 overflow-y-auto p-16 ">
        <ul className="space-y-6">
          {expenses.map((expense) => (
            <li key={expense.id} className="bg-card p-4 rounded-lg shadow-lg border border-border flex justify-between items-center">
              <div>
                <h3 className="text-lg font-medium">{expense.name}</h3>
                <p className="text-muted-forground">{currency.symbol}{expense.amount.toFixed(2)} - {format(expense.date, "dd/MM/yyyy")}</p>
              </div>
